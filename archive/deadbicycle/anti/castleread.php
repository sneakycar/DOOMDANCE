...
// $Id: read.php,v 1.82 2002/02/12 19:53:16 yayel Exp $
  ...
  if($action!=0 && ($action==1 || $action==2)){
    ...
    switch($action){
      case 2:
        ...
        if ($ForumMultiLevel==2){
            $sSQL="Select thread, id from $ForumTableName where modifystamp < $thms $limitApproved order by id asc $limit";
        } else {
            $sSQL="Select thread, id from $ForumTableName where thread<$thread and thread>$cutoff_thread and id=thread $limitApproved order by id asc $limit";
        }
        break;
      case 1:
        $cutoff_thread=$thread+$cutoff;
        if ($ForumMultiLevel==2){
            ...
            $sSQL="Select thread, id from $ForumTableName where thread<$cutoff_thread and thread>$thread and id=thread $limitApproved order by id asc $limit";
        }
        break;
    }
  ...
  $sSQL = "Select * from $ForumTableName where thread=$thread $limitApproved order by id asc";
  ...
  if ($$phflat) {
    $sSQL = "SELECT $ForumTableName.id AS id, $ForumTableName.thread AS thread, body from $ForumTableName, ".$ForumTableName."_bodies WHERE $ForumTableName.approved = 'Y' AND $ForumTableName.thread = ".$thread." AND $ForumTableName.id = ".$ForumTableName."_bodies.id order by id asc";
  } else {
...
<tr>
    <td bgcolor="#ffffff" width="148" valign="top"><font color="<?php echo $ForumTableBodyFontColor2; ?>"><?php echo $lNotFound; ?></td>
</tr>
<table width="148" cellspacing="0" cellpadding="0" border="0">
</table>
<?php }else{ ?>
  ...
  while(is_array($head_row) && is_array($body_row)){
    if($head_row["id"]==$body_row["id"]){
      ...
      if($head_row["userid"]>0 && isset($users[$head_row["userid"]])){
        ...
        $email=$user["email"];
        $host="";
        $author = "<a href=\"$forum_url/profile.$ext?f=$ForumId&id=$head_row[userid]$GetVars\" class=\"feed\" target=\"madness\">$author</a>";
