...
// $Id: read.php,v 1.82 2002/02/12 19:53:16 yayel Exp $
  ...
  if($action!=0 && ($action==1 || $action==2)){
    ...
    switch($action){
      case 2:
        ...
        if ($ForumMultiLevel==2){
            $sSQL="Select thread, id from $ForumTableName where modifystamp < $thms $limitApproved order by id desc $limit";
        } else {
            $sSQL="Select thread, id from $ForumTableName where thread<$thread and thread>$cutoff_thread and id=thread $limitApproved order by id desc $limit";
        }
        break;
      case 1:
        $cutoff_thread=$thread+$cutoff;
        if ($ForumMultiLevel==2){
            ...
            $sSQL="Select thread, id from $ForumTableName where thread<$cutoff_thread and thread>$thread and id=thread $limitApproved order by id desc $limit";
        }
        break;
    }
  ...
  $sSQL = "Select * from $ForumTableName where thread=$thread $limitApproved order by id desc";
  ...
  if ($$phflat) {
    $sSQL = "SELECT $ForumTableName.id AS id, $ForumTableName.thread AS thread, body, artist, rating from $ForumTableName, ".$ForumTableName."_bodies WHERE $ForumTableName.approved = 'Y' AND $ForumTableName.thread = ".$thread." AND $ForumTableName.id = ".$ForumTableName."_bodies.id ORDER BY id desc";
  } else {
...
$msg_artist = new query($DB, $sSQL);

  $rec=$msg_artist->getrow();
  While(is_array($rec)){
    $artists[]=$rec;
    $rec=$msg_artist->getrow();
  }
  ...
  $header_rows=count($headers);
  $artist_rows=count($artists);

$msg_rating = new query($DB, $sSQL);

  $rec=$msg_rating->getrow();
  While(is_array($rec)){
    $ratings[]=$rec;
    $rec=$msg_rating->getrow();
  }
  ...
  //////////////////////////
    ...
    if($ForumSecurity){
      if(!empty($phorum_auth)){
        ...
        addnav($menu2, $lMyProfile, "feedprofile.$ext?f=$f&id=$phorum_user[id]$GetVars");
      }
      else{
...
<table bgcolor="white" class="PhorumListTable" width="<?php echo $ForumTableWidth; ?>" cellspacing="0" cellpadding="1" border="0">
<tr>
    <td"><FONT color="<?php echo $ForumTableHeaderFontColor; ?>">&nbsp;<?php echo $lViolationTitle; ?></font></td>
</tr>
<tr>
    <td valign="TOP"><table width="100%" cellspacing="0" cellpadding="2" border="0">
<tr>
    <td width="100%" valign="top"><font color="<?php echo $ForumTableBodyFontColor2; ?>"><?php echo $lNotFound; ?></td>
</tr>
</table>
</td>
</tr>
...
<?php }else{
  ...
  $artist_row=@current($artists);
  $rating_row=@current($ratings);
  while(is_array($head_row) && is_array($body_row)){
    if($head_row["id"]==$body_row["id"]){
      ...
      if($head_row["userid"]>0 && isset($users[$head_row["userid"]])){
        ...
        $email=$name["email"];
        $host="";
        $author = "<a class=\"author\" target=\"madness\" href=\"$forum_url/feedprofile.$ext?f=$ForumId&id=$head_row[userid]$GetVars\">$author</a>";
        // replace sig
        $sig=$name["signature"];
      }
      else{
...
<table border="0" cellpadding="1" cellspacing="0" width="297">
...
<?php
    ...
    if ($AllowAttachments && $ForumAllowUploads == 'Y') {
      ...
      while($rec=$q->getrow()){
        ...
        echo "$lFormAttachment:&nbsp; <a href=\"$forum_url/download.$ext/$num,$rec_id,$rec[id]/$rec[filename]\">$rec[filename]</a> ($size)\n";
      }
    }
    echo '';
...
?>
<B><?php echo $rating; ?></B>&nbsp;</TD></TR><TR><TD><B>Name: <?php echo $artist; ?> </B></TR><TR><TD><B>What this fellow thinks:</B> <?php echo $body; ?></TD></TR>
</td>
</tr>
<TR>
...
<?php
      ...
      $artist_row=next($artists);
      if(is_array($artist_row)){

      $rating_row=next($ratings);
      if(is_array($rating_row)){
...
<TABLE colspan="3" width="295" border="0" cellspacing="0" cellpadding="0"><TR HEIGHT="8"><TD colspan="3">&nbsp;</TD></TR><TR align="left"><TD><A HREF="http://www.deaddeaddead.com/anti/feeding.php?f=7&i=9&t=9" target="feeder" onmouseover="window.status='back to the good old fashioned news';return true" onmouseout="window.status='';return true"><img src="../../../img/file2.gif" width="22" height="19" border="0"></A><img src="../../../img/file.gif" width="22" height="19" border="0">
<?php if(!empty($phorum_user["moderator"])){ ?>

<?php echo "<a target=\"madness\" href=\"makenoise.php?f=$f&i=$id&s=$subject&t=$t$GetVars\">";?><img src="../../../img/newfeed.gif" width="21" height="19" border="0"></A>
<?php echo "<a target=\"madness\" href=\"$forum_url/feedmoderator.php?mod=edit&f=$ForumId&i=$rec_id&t=$thread$GetVars\">";?><img src="../../../img/editfeed.gif" width="21" height="19" border="0"></a>
<a href="javascript:delmsg('<?php echo "$forum_url/moderator.$ext?mod=delete&f=$ForumId&i=$rec_id&t=$thread$GetVars";?>')"><img src="../../../img/trash.gif" width="21" height="19" border="0"></a>
