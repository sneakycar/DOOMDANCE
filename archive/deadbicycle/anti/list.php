<?php
  ...
  //////////////////////////
    ...
    if($$phcollapse==0){
      addnav($menu2, $n3_gif, "$list_page.$ext?f=$num&collapse=1$GetVars");
    }
    else{
      addnav($menu2, $n4_gif, "$list_page.$ext?f=$num&collapse=0$GetVars");
    }
    ...
    // Mark all read
    addnav($menu1, $lMarkRead, "$list_page.$ext?f=$num$GetVars&r=1");

    // Log Out/Log In
    if($ForumSecurity){
      if(!empty($phorum_auth)){
        addnav($menu3, $lLogOut, "login.$ext?logout=1&f=$f$GetVars");
        addnav($menu3, $lMyProfile, "profile.$ext?f=$f&id=$phorum_user[id]$GetVars");
      }
      else{
        addnav($menu3, $lLogIn, "login.$ext?f=$f$GetVars");
      }
    }
    ...
    if ($ForumMultiLevel==2) {
      if($action!=0) {
        // Newer
        addnav($menu, $none, "$list_page.$ext?f=$num&t=$max&a=1$GetVars");
      }
    } elseif ($action!=0) {
       //Newer
      addnav($menu, $OlderMessages, "$list_page.$ext?f=$num&t=$max&a=1$GetVars");
    }
    // Older
    addnav($menu, $next1_gif, "$list_page.$ext?f=$num&t=$min&a=2$GetVars");
    $nav=getnav($menu);
    $TopRightNav=$nav;
    $LowRightNav=$nav;
...
?>
<DIV ALIGN="CENTER"><table width="85%" cellspacing="0" cellpadding="0" border="0">
</table></DIV>
<?php
...
<DIV ALIGN="CENTER"><table width="85%" cellspacing="0" cellpadding="0" border="0">
<tr><TD>&nbsp;</TD></TR><TR>
  <td width="60%" align="left" nowrap="nowrap" bgcolor="#cce45b"><?php echo $LowLeftNav; ?></TD>
  <td align="right" width="40%" bgcolor="#cce45b"><div class=nav><?php echo $LowRightNav; ?></div></td>
</tr>
</table></DIV>
<?php
