<?php
  ...
  $settings_dir="./love/settings";  // no ending slash
...
//////////////////////////////////////////////////////////////////////////////////////////
  ...
  function getnav($var, $splitter="&nbsp;&nbsp;::&nbsp;&nbsp;", $usefont=true){
    ...
    while(list($text, $url)=each($var)){
      ...
      $menu[]="<a href=\"$url\" target=\"help\">$text</a>";
    }
  ...
  function phorum_get_file_name($type)
  {
    ...
    switch($type){
        case "css":
            ...
            break;
        case "header":
            $file="$PHORUM[include]/noiseheader.php";
  ...
  function phorum_check_login($name, $pass)
  {
    global $q, $DB, $PHORUM;
    ...
    $id=0;
    $SQL="Select id from $PHORUM[auth_table] where name='$name' and password='$md5_pass'";
    $q->query($DB, $SQL);
    if($q->numrows()==0 && function_exists("crypt")){
        // check for old crypt system
        $crypt_pass=crypt($pass, substr($pass, 0, CRYPT_SALT_LENGTH));
        $SQL="Select id from $PHORUM[auth_table] where name='$name' and password='$crypt_pass'";
        $q->query($DB, $SQL);
        if($q->numrows()>0){
            // update password to md5.
            $SQL="Update $PHORUM[auth_table] set password='$md5_pass' where name='$name'";
            $q->query($DB, $SQL);
        }
    }
  ...
  function phorum_check_loginfeed($name, $pass)
  {
    global $q, $DB, $PHORUM;
    ...
    $id=0;
    $SQL="Select id from $PHORUM[auth_table] where name='gerald' and password='$md5_pass'";
    $q->query($DB, $SQL);
    if($q->numrows()==0 && function_exists("crypt")){
        // check for old crypt system
        $crypt_pass=crypt($pass, substr($pass, 0, CRYPT_SALT_LENGTH));
  ...
  function phorum_session_id($name, $password)
  {
    return md5($name.$password.microtime());
  }
