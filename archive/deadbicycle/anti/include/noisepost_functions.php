<?php
  ...
  function violation(){
    global $num,$author,$email,$subject,$body,$ip,$artist,$rating,$host,$violation_page,$ext,$GetVars,$ForumName,$ForumModEmail,$PhorumMail;
  ...
  function check_data($host, $author, $subject, $body, $email, $artist, $rating) {
    global $lNoAuthor, $lNoSubject, $lNoBody, $lNoEmail, $lnoartist, $lnorating, $lRegisteredName;
  ...
  function censor($author, $subject, $email, $body, $artist, $rating) {
    global $include_path, $ForumConfigSuffix;
...
if(strstr(strtoupper($artist), strtoupper($sWord))){
          if(strtoupper($artist)==strtoupper($sWord)) $artist=$blurb;
          $artist = eregi_replace("^$sWord([^a-zA-Z])", "$blurb\\1", $artist);
          $artist = eregi_replace("([^a-zA-Z])$sWord$", "\\1$blurb", $artist);
          while(eregi("([^a-zA-Z])($sWord)([^a-zA-Z])", $artist)){
            $artist = eregi_replace("([^a-zA-Z])($sWord)([^a-zA-Z])", "\\1$blurb\\3", $artist);
          }
        }
if(strstr(strtoupper($rating), strtoupper($sWord))){
          if(strtoupper($rating)==strtoupper($sWord)) $rating=$blurb;
          $rating = eregi_replace("^$sWord([^a-zA-Z])", "$blurb\\1", $rating);
          $rating = eregi_replace("([^a-zA-Z])$sWord$", "\\1$blurb", $rating);
          while(eregi("([^a-zA-Z])($sWord)([^a-zA-Z])", $rating)){
            $rating = eregi_replace("([^a-zA-Z])($sWord)([^a-zA-Z])", "\\1$blurb\\3", $rating);
          }
        }
      }
    }
    return(array($author, $subject, $email, $body, $artist, $rating));
  }
  ...
  function check_dup() {
    ...
    global $author, $subject, $body, $artist, $rating;
    ...
    if($ForumCheckDup){
      ...
      if($q->numrows()>0){
        ...
        $sSQL="Select id from $ForumTableName"."_bodies where id in ($ids) and body='$body' and artist='$artist' and rating='$rating' ";
        $q->query($DB, $sSQL);
        if($q->numrows()>0) {
    return(true);
  }
  ...
  function post_to_database() {
    ...
    global $thread, $subject, $inreplyto, $parent, $author, $body, $email, $artist, $rating;
    global $image, $datestamp, $host, $email_reply, $attachment_name, $msgid;
    global $plain_author, $plain_subject, $plain_body, $plain_artist, $plain_rating;
    ...
    $sSQL = "Insert Into $ForumTableName"."_bodies (id, body, thread, artist, rating) values ($id, '$body', $thread, '$artist', '$rating')";
    $q->query($DB, $sSQL);
    if($err=$q->error()){
      echo($err."<br />$sSQL");
    }
...
//        mail("$ForumName <$ForumEmailList>", "$plain_subject", $ebody, $headers);
      ...
      if($replies){
        $headers.="\nBCC: $replies";
        mail("", "$plain_subject", $ebody, $headers);
      }
//    }
  }
