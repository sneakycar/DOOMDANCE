<?php
  ...
  if($ForumSecurity > SEC_OPTIONAL && empty($phorum_auth)){
    header("Location: http://www.deadbicycle.com/anti/noise.php?f=9&i=14&t=14");
    exit();
  }

  require "$include_path/noisepost_functions.php";
  ...
  $IsError = @check_data($host, $author, $subject, $body, $email, $artist, $rating);
  ...
  //////////////////////////
    ...
    include "$include_path/noiseform.php";
  ...
  list($author, $subject, $email, $body, $artist, $rating) = censor($author, $subject, $email, $body, $artist, $rating);
  ...
  $plain_artist=stripslashes(strip_tags($artist));
  $plain_rating=stripslashes(strip_tags($rating));
