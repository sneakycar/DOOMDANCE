...
//////////////////////////////////////////////////////////////////////////////////////////
  ...
  function phorum_get_file_name($type)
  {
    ...
    switch($type){
        case "css":
            ...
            break;
        case "header":
            $file="$PHORUM[include]/header5.php";
  ...
  function phorum_check_loginfeed($user, $pass)
  {
    global $q, $DB, $PHORUM;
    ...
    $id=0;
    $SQL="Select id from $PHORUM[auth_table] where username='gerald' and password='$md5_pass'";
    $q->query($DB, $SQL);
    if($q->numrows()==0 && function_exists("crypt")){
        // check for old crypt system
        $crypt_pass=crypt($pass, substr($pass, 0, CRYPT_SALT_LENGTH));
