  ...
  function create_table(&$DB, $table, $table_name){
    ...
    switch($table){
      case "main":
        ...
        $q->query($DB, $sSQL);
        if(!$q->error()){
          $sSQL="CREATE TABLE ".$table_name."_bodies (id int unsigned DEFAULT '0' NOT NULL AUTO_INCREMENT, body text DEFAULT '' NOT NULL, artist text DEFAULT '' NOT NULL, rating text DEFAULT '' NOT NULL, thread int unsigned DEFAULT '0' NOT NULL, PRIMARY KEY (id), KEY thread (thread))";
          $q->query($DB, $sSQL);
          if($q->error()){
            $errormsg = $q->error();
